...
monitor_position(mrb_state* mrb, mrb_value self)
{
  ...
  int xpos;
  int ypos;
  glfwGetMonitorPos(mrb_glfw3_get_monitor(mrb, self), &xpos, &ypos);
...
monitor_physical_size(mrb_state* mrb, mrb_value self)
{
  ...
  int widthMM;
  int heightMM;
  glfwGetMonitorPhysicalSize(mrb_glfw3_get_monitor(mrb, self), &widthMM, &heightMM);
...
monitor_set_gamma(mrb_state* mrb, mrb_value self)
{
  ...
  glfwSetGamma(mrb_glfw3_get_monitor(mrb, self), gamma);
